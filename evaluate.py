...
def generate_captions_in_batches(images, batch_size):
    all_captions = []
    for batch in tqdm(chunk_images(images, batch_size)):
        # Preprocess the current batch
        processed_images = [vis_processor(Image.open(BytesIO(image))).unsqueeze(0).to("cuda") for image in batch]
...
if __name__ == "__main__":
    ...
    test_data_path = "/home/charles/VLM/data/facad_test_data.parquet"
    ...
    captions = generate_captions_in_batches(image_bytes, batch_size=batch_size)
